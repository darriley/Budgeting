...
function displayBudgetBreakdown(budget) {
  ...
  if (budget >= 0) {
    budgetBreakdown.innerHTML += `<li>Remaining Budget: 
₱${budget.toFixed(2)}</li>`;
  } else {
    budgetBreakdown.innerHTML += `<li>Over Budget: 
₱${Math.abs(budget).toFixed(2)}</li>`;
  }

  if (expensesArray.length > 0) {
    budgetBreakdown.innerHTML += '<li>Expense Breakdown:</li>';
    expensesArray.forEach((expense) => {
      budgetBreakdown.innerHTML += `<li>Date: ${expense.date}, ${expense.name}: 
₱${expense.amount.toFixed(2)}</li>`;
    });
  }
}
// ... (previous code)
...
function displayAllBudgetBreakdowns() {
  ...
  expensesArray.forEach((budgetEntry) => {
    ...
    if (budgetEntry.budget >= 0) {
      budgetBreakdown.innerHTML += `<li>Remaining Budget: 
₱${budgetEntry.budget.toFixed(2)}</li>`;
    } else {
      budgetBreakdown.innerHTML += `<li>Over Budget:
$${Math.abs(budgetEntry.budget).toFixed(2)}</li>`;
    }

    if (budgetEntry.expenses.length > 0) {
      budgetBreakdown.innerHTML += '<li>Expense Breakdown:</li>';
      budgetEntry.expenses.forEach((expense) => {
        budgetBreakdown.innerHTML += `<li>${expense.name}:
₱${expense.amount.toFixed(2)}</li>`;
      });
    }
  });
